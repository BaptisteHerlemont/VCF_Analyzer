...
input_vcf_file <- "synthetic_20ind_10k.vcf"  # Change "YourInputFileName.vcf.gz" to the name of your VCF file
output_csv_file_bySite <- "synthetic_20ind_10k_by_site.tsv"  # Change "OutputFileNameBySite.csv" to your desired output file name for site data
output_csv_file_byGeno <- "synthetic_20ind_10k_by_geno.tsv"  # Change "OutputFileNameByGeno.csv" to your desired output file name for genotype data


```

```{python VCF, eval=TRUE, include=FALSE}
...
file2_dim_value <- file2_dim[1]
file1_dim_value <- file1_dim[1]

```
To summarize, this VCF file contains **`r file2_dim_value`** genotypes and **`r file1_dim_value`** sites in total.




### Genotype-level preview
...
Note: The reference count is not displayed on the bar chart.
...
ggplot(totals, aes(x = Variable, y = Total/1e6, fill = type)) +
  ...
  labs(title = "Total calls across sites",
       x = "Call category",
       y = "Total count (millions)") +
  scale_fill_manual(values = c("snp" = "#0072B2", "indel" = "#E69F00"), name = "Variant type") +
...
boxplotindel <- Cassat2_Scale8_color[1]
 ...
 ggplot(file_sites, aes(x = type, y = het, fill = type)) +
    ...
    geom_boxplot(aes(color = type), width = 0.1, alpha = 1, outlier.shape = NA, show.legend = FALSE) +
    scale_fill_manual(values = c("snp" = Cassat2_SNPHET_color, "indel" = Cassat2_INDELHET_color), name = "Variant type") +
    scale_color_manual(values = c("snp" = boxplotsnp, "indel" = boxplotindel), guide = "none") +
    xlab("Variant type") +
    ylab("Heterozygous calls per site") +
    labs(title = "Distribution of heterozygous calls by variant type") +
    theme_minimal() + theme(panel.grid = element_blank())
...
## Genotype-Level Statistics
### Missing calls by genotype (circular view)
This circular chart helps highlight genotypes with elevated missingness.
...
ggplot(data, aes(x = Genotype)) +
  ...
  scale_fill_manual(
    ...
    ggplot(data, aes(x = Genotype)) +
      ...
      coord_polar(start = 0) +
      scale_fill_manual(values = c(SNP = Troy_SNP_color), name = "Variant type", labels = c("SNP" = "SNP"))
} else if (has_indel) {
    ggplot(data, aes(x = Genotype)) +
      geom_bar(aes(y = INDEL_missing, fill = "INDEL"), width = 1.1, stat = "identity", position = position_dodge(width = 1.1), alpha = 0.9) +
      ylim(-max_value, max_value) +
      ...
      coord_polar(start = 0) +
      scale_fill_manual(values = c(INDEL = Java_INDEL_color), name = "Variant type", labels = c("INDEL" = "INDEL"))
}
...
### Missing calls by genotype (ordered view)
This ordered chart is useful for choosing a filtering threshold.
```{r, barplot, echo=TRUE}
...
ggplot(file_genotypes, aes(x = Genotype)) +
  ...
  theme_minimal() + theme(panel.grid = element_blank()) +
  ggtitle("Missing calls by genotype") +
  labs(x = "Genotype", y = "Missing calls") +
  scale_fill_manual(values = c(SNP_Missing = Derain_SNPMISSING_color, INDEL_Missing = Morgenstern_INDELMISSING_color), name = "Missing type") +
  ...
  guides(
    fill = guide_legend(title = "Missing type", direction = "vertical"),
  ...
  ggplot(file_genotypes_sorted, aes(x = reorder(Genotype, INDEL_missing))) +
    ...
    theme_minimal() + theme(panel.grid = element_blank()) +
    ggtitle("Missing INDEL calls by genotype") +
    labs(x = "Genotype", y = "Missing calls") +
    scale_fill_manual(values = c(INDEL_Missing = Morgenstern_INDELMISSING_color), name = "Missing type") +
...
cat("Summary of genotypes exceeding different missingness thresholds")
...
if (is.null(summary_data)) {
  cat("No missing SNP or INDEL values to summarize.")
} else {
...
### Missing rate per site
```{r, missing_rate_per_variant, echo=TRUE}
  file_sites_missing <- file_sites %>%
    mutate(
      total = ref + alt + het + missing,
      missing_rate = ifelse(total > 0, missing / total, NA)
    )

  ggplot(file_sites_missing, aes(x = missing_rate)) +
    geom_histogram(bins = 50, fill = met.brewer(name = "Java", n = 1), color = "white") +
    scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
    labs(
      title = "Missing rate per site",
      x = "Missing rate",
      y = "Number of sites"
    ) +
    theme_minimal() + theme(panel.grid = element_blank())
...
### Missing rate per site by variant type
```{r, missing_rate_per_variant_by_type, echo=TRUE}
  ggplot(file_sites_missing, aes(x = missing_rate, fill = type)) +
    geom_histogram(bins = 40, position = "stack", alpha = 1, color = NA) +
    scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
    scale_fill_manual(values = c("snp" = "#0072B2", "indel" = "#E69F00"), name = "Variant type") +
    labs(
      title = "Missing rate per site by variant type",
      x = "Missing rate",
      y = "Number of sites"
    ) +
    theme_minimal() + theme(panel.grid = element_blank())
